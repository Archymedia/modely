...
# -*- coding: utf-8 -*-
"""
1D-CNN pipeline for predicting next-day Z-scored simple returns (SP100) + trading backtest.
...
'n_iter': 20,                   # Random search iterations
    ...
    # HYPERPARAMETER SEARCH SPACE (CNN)
    'search_space': {
        'conv_blocks': [1, 2],             # number of Conv1D blocks
        'filters': [32, 64, 128],          # filters per conv layer
        # temporal kernel
        'learning_rate': [1e-4, 3e-4, 1e-3],
        'window': [20, 40, 60]             # shared with RNN/LSTM
    },
...
# ---------------------- 4. CNN Model ----------------------
def build_cnn(input_dim: int, timesteps: int, conv_blocks: int, filters: int, kernel_size: int,
              lr: float, l2_reg: float, dropout_rate: float) -> keras.Model:
    reg = regularizers.l2(l2_reg)
    inp = keras.Input(shape=(timesteps, input_dim))
    x = inp
    ...
    model = keras.Model(inputs=inp, outputs=out, name="CNN1D_regressor")
...
def make_report_png(out_dir: str, train_hist: Dict, equity_curve: pd.DataFrame, best_params: Dict,
    ...
    png_path = os.path.join(out_dir, "cnn_recap.png")
...
def main():
    ...
    out_dir = os.path.abspath("./cnn_outputs")
    safe_makedirs(out_dir)
