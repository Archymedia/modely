...
# -*- coding: utf-8 -*-
"""
MLP pipeline for predicting next-day Z-scored simple returns (SP100) + trading backtest.
...
'patience': 10,
'cv_folds': 3,
'n_iter': 18,                   # Random search iterations

    # HYPERPARAMETER SEARCH SPACE (MLP)
    'search_space': {
        'layers': [1, 2],                   # number of hidden layers
        'units': [64, 128, 256],            # neurons per hidden layer
...
def day_count_to_annualization_factor(n_days: int) -> float:
    # Assume ~252 trading days per year
    return np.sqrt(252 / max(n_days, 1))

def to_daily_rf(rf_annual: float) -> float:
    # Convert annual risk-free to daily (compounding)
...
def make_time_splits(df: pd.DataFrame, train_end: str, test_start: str, n_folds: int = 5) -> Dict:
    ...
    # Build folds inside training period by calendar time
    dates = np.sort(train_df["Date"].unique())
    folds = []
    # Create rolling folds: split the timeline into n_folds+1 chunks and use expanding train
    chunk_edges = np.linspace(0, len(dates), n_folds+1, dtype=int)
    # We'll use each edge as the end of validation chunk (skip the first which is 0)
    for i in range(1, len(chunk_edges)):
        ...
        # Define train as all strictly before val_start
...
def per_id_train_scalers(train_df: pd.DataFrame, feature_cols: List[str]) -> Dict[str, StandardScaler]:
    ...
    for id_, g in train_df.groupby("ID"):
        ...
        if mask.sum() >= 5:
            ...
            # Fallback scaler (will just center around zero)
...
def apply_scalers(df: pd.DataFrame, feature_cols: List[str], scalers: Dict[str, StandardScaler]) -> pd.DataFrame:
    ...
    for id_, g in df.groupby("ID"):
        ...
        sc = scalers.get(id_)
        if sc is None:
            # Use a neutral scaler if unseen ID (shouldn't happen here)
...
def time_cv_evaluate(train_df: pd.DataFrame, folds: List[Tuple[np.ndarray, np.ndarray]], feature_cols: List[str],
                     ...
                     out_dir: str) -> Tuple[float, Dict]:
    """Return average val MSE across folds and history for the last fold (for plotting)"""
    val_losses = []
    last_history = None
...
def random_search(train_df: pd.DataFrame, folds: List[Tuple[np.ndarray, np.ndarray]], feature_cols: List[str],
                  search_space: Dict, fixed: Dict, cfg: Dict, out_dir: str) -> Dict:
    ...
    for i, cand in enumerate(candidates, 1):
        ...
        try:
            avg_val, info = time_cv_evaluate(train_df, folds, feature_cols, cand, fixed,
                                             ...
                                             l2_reg=l2_reg, dropout=dropout, out_dir=out_dir)
...
# ---------------------- 5. Train Final Model on Full Train ----------------------
def train_final(train_df: pd.DataFrame, feature_cols: List[str], best_params: Dict, fixed: Dict, cfg: Dict) -> Tuple[keras.Model, Dict]:
    ...
    hist = model.fit(
        X_tr, y_tr,
        validation_split=0.1,  # time-agnostic split inside train; acceptable after CV; we keep ES mild
...
# ---------------------- 6. Prediction & Daily Ranking ----------------------
def predict_all(model: keras.Model, df_scaled: pd.DataFrame, feature_cols: List[str]) -> pd.Series:
    X = df_scaled[feature_cols].values
    yhat = model.predict(X, batch_size=1024, verbose=0).reshape(-1)
    return pd.Series(yhat, index=df_scaled.index, name="y_pred")
...
def simulate_trading(df: pd.DataFrame, preds: pd.Series, strategy: Dict) -> Dict:
    ...
    # Daily universe filter
    unique_dates = np.sort(df["Date"].unique())
    # We need to form signal on D, enter at D+1
    # Build map Date -> IDs sorted by prediction
    daily_ranks = {}
    ...
    # Positions opened at open D+1, closed at first barrier hit
    equity_curve = []
    daily_pnl = []
    open_positions = []  # list of dicts: {id, entry_date, entry_open, side, pt, sl}
    id_date_index = {(row.Date, row.ID): idx for idx, row in df[["Date","ID"]].itertuples()}

    # Helper to process barrier hit from D+1 onward
    def process_position(pos, day_slice):
        # day_slice contains rows for that ID across subsequent days
        # conservative rule: if both PT and SL hit on same day, assume SL is hit first
        ...
        # If no barrier was ever hit, close at last available close
        r = day_slice.iloc[-1]
        exit_price = r["CloseAdj"]
        exit_date = r["Date"]
        ret = (exit_price - entry_open) / entry_open if pos["side"] == "long" else (entry_open - exit_price) / entry_open
        return exit_date, ret

    # Iterate days, form signals, enter next day
    date_to_row = {d: df[df["Date"] == d] for d in unique_dates}
    daily_returns = []  # portfolio daily return (EW across all open positions? here: realized when closed)
    # We'll compute day-by-day MTM by summing P&L realized that day (simplified).
    pnl_by_day = {d: 0.0 for d in unique_dates}

    for i, d in enumerate(unique_dates[:-1]):  # last day cannot open new positions
        ...
        if ranks:
            ...
            for sid in ranks["longs"]:
                ...
                # Determine barrier exit using future path
    ...
    # Build equity curve by cumulating daily P&L (EW, unit notional per trade)
    # Normalize by number of simultaneously opened positions per day (n_long + n_short)
    positions_per_day = (nL + nS)
    for d in unique_dates:
        daily_ret = pnl_by_day.get(d, 0.0)
        # Scale by positions_per_day to represent average daily return of the strategy
        daily_ret /= max(positions_per_day, 1)
        daily_pnl.append(daily_ret)

    equity = np.cumprod(1 + np.array(daily_pnl))
    equity_curve = pd.DataFrame({"Date": unique_dates, "Strategy": equity})
    ...
    bm_daily.append(0.0)  # last day no next day
...
def portfolio_metrics(equity_curve: pd.DataFrame, daily_ret: np.ndarray, bm_daily: np.ndarray, rf_annual: float) -> Dict:
    ...
    # Simple t-stat of mean daily return
...
def make_report_png(out_dir: str, train_hist: Dict, equity_curve: pd.DataFrame, best_params: Dict,
    ...
    png_path = os.path.join(out_dir, "mlp_recap.png")
    ...
    # Vertical line at train_end_date
    xline = pd.to_datetime(train_end_date)
    plt.axvline(x=xline, linestyle="--")
    plt.legend()

    # 3) Table of best hyperparams and model info (bottom-left)
    plt.subplot(2,2,3)
...
# ---------------------- 10. Grading with Emoji ----------------------
def grade_strategy(port_metrics: Dict) -> Dict:
    # Simple composite score from CAGR, Sharpe, and -MaxDD (scaled)
    cagr = port_metrics.get("CAGR", 0.0)
    sharpe = port_metrics.get("Sharpe", 0.0)
    maxdd = abs(port_metrics.get("MaxDrawdown", 0.0))
    # Normalize-ish scoring
    score = 0.0
    score += max(0.0, min(10.0, cagr*100)) * 0.2     # 20% weight
    score += max(0.0, min(10.0, sharpe*2)) * 0.5     # 50% weight
    score += max(0.0, min(10.0, (1.0-maxdd)*10)) * 0.3  # 30% weight, penalize deep drawdowns
...
def main():
    ...
    # Fit per-ID scalers on train and transform train/test
    ...
    # Target z-score using TRAIN stats only
    ...
    # Ensure folds index refers to train_scaled index (already built from train_df)
    # Random search
    best = random_search(train_scaled, folds, feature_cols, HYPERPARAMS["search_space"],
                         HYPERPARAMS["fixed_params"], HYPERPARAMS, out_dir)
    ...
    # Predictions for diagnostics (train/test regression metrics)
    yhat_train = predict_all(model, train_scaled, feature_cols).values
    yhat_test  = predict_all(model, test_scaled, feature_cols).values
    reg_train = regression_metrics(train_scaled["target_z"].values, yhat_train)
    reg_test  = regression_metrics(test_scaled["target_z"].values, yhat_test)

    # Trading simulation on full period using predictions day by day
    preds_all = predict_all(model, df_all_scaled, feature_cols)
