...
# -*- coding: utf-8 -*-
"""
SimpleRNN pipeline for predicting next-day Z-scored simple returns (SP100) + trading backtest.
...
'n_iter': 20,                   # Random search iterations
    ...
    # HYPERPARAMETER SEARCH SPACE (RNN)
    'search_space': {
        'layers': [1, 2],                   # number of SimpleRNN layers
        'units': [32, 64, 128],             # neurons per layer
        'learning_rate': [1e-4, 3e-4, 1e-3],
        'window': [20, 40, 60]              # shared with LSTM/CNN for fair comparison
    },
...
# ---------------------- 3. Sequence Building ----------------------
def build_sequences(df: pd.DataFrame, feature_cols: List[str], window: int) -> Tuple[np.ndarray, np.ndarray, List[Tuple[int,int]]]:
    """
    Build sequences per ID. Returns X (N,L,F), y (N,), and meta index list (row positions in df).
    y is target_z at the sequence end date. Features are already lagged by 1 day.
    """
    X_list, y_list, meta = []
    X_list = []
    y_list = []
    ...
    for id_, g in df.groupby("ID"):
        ...
        for t in range(window-1, len(g)):
            ...
            X_list.append(x_win)
            y_list.append(y_t)
            meta.append((id_, idx[t]))  # (ID, row_index_at_sequence_end)
    if not X_list:
...
# ---------------------- 4. RNN Model ----------------------
def build_rnn(input_dim: int, timesteps: int, layers_n: int, units: int, lr: float, l2_reg: float, dropout_rate: float) -> keras.Model:
    reg = regularizers.l2(l2_reg)
    model = keras.Sequential(name="SimpleRNN_regressor")
    model.add(layers.Input(shape=(timesteps, input_dim)))
    for i in range(layers_n):
        return_seq = (i < layers_n - 1)
        model.add(layers.SimpleRNN(units, activation="tanh", return_sequences=return_seq,
...
def time_cv_evaluate(train_df: pd.DataFrame, folds: List[Tuple[np.ndarray, np.ndarray]], feature_cols: List[str],
                     cfg: Dict, fixed: Dict, epochs: int, patience: int, batch_size: int, l2_reg: float, dropout: float) -> Tuple[float, Dict]:
    val_losses = []
    last_history = None
...
def predict_over_all_dates(model: keras.Model, df_scaled: pd.DataFrame, feature_cols: List[str], window: int) -> pd.Series:
    """
    Build sequences over the full df and map predictions back to the df index of the sequence end date.
    Rows without enough history keep NaN predictions.
    """
    ...
    # meta contains (ID, row_index_at_end)
...
def make_report_png(out_dir: str, train_hist: Dict, equity_curve: pd.DataFrame, best_params: Dict,
    ...
    png_path = os.path.join(out_dir, "rnn_recap.png")
...
def main():
    ...
    out_dir = os.path.abspath("./rnn_outputs")
    safe_makedirs(out_dir)
    ...
    # Predictions for regression diagnostics
    X_tr, y_tr, meta_tr = build_sequences(train_scaled, feature_cols, best_params["window"])
    X_te, y_te, meta_te = build_sequences(test_scaled, feature_cols, best_params["window"])
    ...
    # Trading simulation using per-date predictions
