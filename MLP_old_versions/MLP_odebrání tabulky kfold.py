...
HYPERPARAMS = {
    ...
    'final_model_epochs': 200,       # Maximální počet epoch pro FINÁLNÍ trénink (zvýšeno z 3)
    'cv_epochs': 50,                 # Počet epoch pro CROSS-VALIDATION (zvýšeno z 3)
    'patience': 30,                  # Early stopping patience (zvýšeno z 2)
...
def create_comprehensive_visualization(history, fold_histories, portfolio_tracking_df, best_params, train_metrics, test_metrics, trading_metrics):
    ...
    # 1. Final Model Training Progress (bez k-fold duplicity)
    ax1 = fig.add_subplot(gs[0, :2])
    if history:
        # Zobrazíme jen final model training history
        ax1.plot(history.history['loss'], label='Training Loss', linewidth=2, color='darkblue')
        ax1.plot(history.history['val_loss'], label='Validation Loss', linewidth=2, color='goldenrod')
...
def main():
    ...
    try:
        ...
        output_file = create_comprehensive_visualization(
            history, None, portfolio_tracking_df, best_params, train_metrics, test_metrics, trading_metrics)
